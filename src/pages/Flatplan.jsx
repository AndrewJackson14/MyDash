...
export const FlatplanPage = ({ pageNum, pageDisplay, pub, adsOnPage, dragId, onDrop, onDropToCell, onRemoveAd, onStartDrag, clientName, pageW, editorialStories, isSelected, sectionSelected, onClick, phLabels, onClientModClick, layoutImageUrl, onOpenLayoutModal, adProjectBySaleId, issueAdDeadline }) => {
  ...
  return <div onClick={onClick} onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); if (dragId) onDrop(dragId, pageNum); }} style={{ position: "relative", width: pageW, height: pH, background: isSelected ? Z.ac + "12" : Z.bg, border: "none", outline: sectionSelected ? `2px solid ${Z.wa}` : "none", borderRadius: R, overflow: "hidden", flexShrink: 0, cursor: "pointer" }}>
    ...
    {/* Page number watermark — drops behind layout image when present.
        Uses pageDisplay (e.g. "A1" for newspapers) when provided, else
        falls back to the raw page number. */}
    <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", zIndex: 0, pointerEvents: "none" }}><span style={{ fontSize: Math.max(18, pageW * 0.18), fontWeight: FW.black, color: "rgba(138,149,168,0.08)" }}>{pageDisplay || pageNum}</span></div>
