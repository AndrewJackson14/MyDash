...
const SalesCRM = (props) => {
  ...
  const openProposal = (clientId) => { setPropClient(clientId || clients[0]?.id || ""); setPropPubs([]); setPropPayPlan(false); setPropStep("build"); setPropName(`Proposal ${new Date().toLocaleDateString()}`); setEditPropId(null); setPropAddPubId(pubs[0]?.id || ""); setPropExpandedPub(null); setPropEmailRecipients([]); setPropEmailMsg(""); setViewPropId(null); setPropMo(true); };
  ...
  return <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
    ...
    {tab === "Proposals" && !viewPropId && <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10, marginBottom: 4 }}>
        ...
        ].map(([l, v, c]) => <div key={l} style={{ ...glass(), borderRadius: R, padding: "10px 14px" }}><div style={{ fontSize: FS.xs, fontWeight: FW.heavy, color: Z.td, letterSpacing: 1, textTransform: "uppercase" }}>{l}</div><div style={{ fontSize: 18, fontWeight: FW.black, color: Z.tx, fontFamily: "'Playfair Display',Georgia,serif" }}>{v}</div></div>)}
      </div>
    ...
    {tab === "Renewals" && (() => {
      ...
      {scored.length === 0 && <GlassCard style={{ textAlign: "center", padding: 20, color: Z.ac, fontSize: 15, fontWeight: FW.bold }}>All caught up — no renewals due</GlassCard>}
      {/* THREE LANES */}
      {[{ label: "Ready to Renew", items: ready, total: totalReady, color: Z.ac, action: "Send Renewal" }, { label: "Warm Up Needed", items: warm, total: totalWarm, color: Z.wa, action: "Schedule Check-in" }, { label: "At Risk", items: atRisk, total: totalAtRisk, color: Z.da, action: "Review Account" }].map(lane => lane.items.length === 0 ? null : <div key={lane.label}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0 4px", borderBottom: `2px solid ${lane.color}` }}><span style={{ fontSize: FS.lg, fontWeight: FW.semi, color: Z.tx, fontFamily: COND }}>{lane.label}</span><span style={{ fontSize: FS.xs, fontWeight: FW.heavy, color: "#fff", background: lane.color, padding: "1px 7px", borderRadius: R }}>{lane.items.length}{lane.total > lane.items.length ? ` of ${lane.total}` : ""}</span></div>
        {lane.items.slice(0, 25).map(s => <div key={s.clientId || s.id} style={{ ...glass(), borderRadius: R, padding: 16, marginTop: 4 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div><span style={{ fontSize: 15, fontWeight: FW.semi, color: Z.tx, fontFamily: COND }}>{cn(s.clientId)}</span><div style={{ fontSize: FS.sm, color: Z.tm }}>${(s.totalSpend || s.amount || 0).toLocaleString()} total · {s.saleCount || 1} orders · {s.pubCount || 1} pub{(s.pubCount || 1) > 1 ? "s" : ""}</div><div style={{ fontSize: FS.xs, color: Z.td }}>Last: {s.lastDate || s.date}</div></div>
            <div style={{ textAlign: "right" }}><div style={{ fontSize: 18, fontWeight: FW.black, color: lane.color }}>{s.score}</div><div style={{ fontSize: FS.micro, color: Z.td, textTransform: "uppercase" }}>score</div></div>
          </div>
          {/* Upsell intelligence */}
          {(() => {
    ...
    {/* PROPOSAL BUILDER */}
    <Modal open={propMo} onClose={closePropMo} title={propStep === "sent" ? "Sent!" : propStep === "email" ? "Send Proposal" : "Build Proposal"} width={1100}>
