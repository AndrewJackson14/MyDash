...
export function DataProvider({ children, localData }) {
  ...
  const loadStories = useCallback(async () => {
    ...
    const { data } = await supabase.from('stories').select('*').order('due_date');
    if (data) setStories(data.map(s => ({ id: s.id, title: s.title, author: s.author, status: s.status, publication: s.publication_id, assignedTo: s.assigned_to || '', dueDate: s.due_date, images: s.images, wordCount: s.word_count, category: s.category, issueId: s.issue_id || '' })));
